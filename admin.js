...
async function criarAluno() {
  ...
  const { error: erroAluno } = await _supabase.from('alunos').insert({
    id: data.user.id, nome, email, concurso_id
  })
...
async function carregarAlunos() {
  const { data: alunos } = await _supabase
    .from('alunos').select('*, concursos(nome)').order('criado_em', { ascending: false })
...
async function adicionarAoPlano() {
  ...
  alert(`✅ ${disciplina} adicionada!${usarRevisao ? `\n📝 Exercícios em ${diasExercicios} dias\n🔄 Revisão em ${diasRevisao} dias` : ''}`)
  renderizarPlano(aluno_id)
  renderizarRevisoes(aluno_id)
}
...
async function copiarPlano() {
  ...
  const { data: itens } = await _supabase.from('plano_aluno').select('disciplina, dia_semana, tempo_minutos, meta_questoes, ordem').eq('aluno_id', aluno_origem)
...
async function carregarDesempenho() {
  ...
  const { data: alunos } = await _supabase.from('alunos').select('*').eq('concurso_id', concurso_id)
